...
def main(_):
    ...
    def eval_in_batches(data, sess):
        """Get all prediction for a dataset by running it in small batches."""
        size = data.shape[0]
        if size < EVAL_BATCH_SIZE:
            raise ValueError("batch size for evals larger than dataset: %d" % size)
        prediction = numpy.ndarray(shape=(size, NUM_LABELS), dtype=numpy.float32)
        for begin in xrange(0, size, EVAL_BATCH_SIZE):
            end = begin + EVAL_BATCH_SIZE
            if end <= size:
                prediction[begin:end, :] = sess.run(
                    eval_prediction,
                ...
                prediction[begin:, :] = batch_predictions[begin - size:, :]
        return prediction
